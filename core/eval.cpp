...
constexpr int16_t mg_pawn_table[64] = {
      0,   0,   0,   0,   0,   0,  0,   0,
...
constexpr int16_t eg_pawn_table[64] = {
      0,   0,   0,   0,   0,   0,   0,   0,
...
constexpr int16_t mg_knight_table[64] = {
...
constexpr int16_t eg_knight_table[64] = {
...
constexpr int16_t mg_bishop_table[64] = {
...
constexpr int16_t eg_bishop_table[64] = {
...
constexpr int16_t mg_rook_table[64] = {
...
constexpr int16_t eg_rook_table[64] = {
...
constexpr int16_t mg_queen_table[64] = {
...
constexpr int16_t eg_queen_table[64] = {
...
constexpr int16_t mg_king_table[64] = {
...
constexpr int16_t eg_king_table[64] = {
...
void init_ps_tables() {
    const int16_t* mg_pst_table[PIECE_TYPE_NB] = {
        nullptr, mg_pawn_table, mg_knight_table,
        mg_bishop_table, mg_rook_table, mg_queen_table,
        mg_king_table
    };
    const int16_t* eg_pst_table[PIECE_TYPE_NB] = {
        nullptr, eg_pawn_table, eg_knight_table,
...
int16_t evaluate(const Board &b) {
    ...
    for (PieceType pt: ALL_PTYPES) {
        Bitboard white = b.pieces(WHITE, pt),
                 black = b.pieces(BLACK, pt);
        while (white) {
            Square sq = Square(pop_lsb(white) ^ 56);
            mg[WHITE] += mg_table[pt][sq];
            eg[WHITE] += eg_table[pt][sq];
            game_phase += gamephaseInc[pt];
        }

        while (black) {
            Square sq = pop_lsb(black);
            mg[BLACK] += mg_table[pt][sq];
            eg[BLACK] += eg_table[pt][sq];
            game_phase += gamephaseInc[pt];
        }
    }
